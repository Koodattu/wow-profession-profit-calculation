...
Database migrations and the bundled profession catalog are applied automatically before the backend starts listening. Blizzard price refreshes then run in the background at minute 5 of every hour. Newly discovered auction items are hydrated in bounded batches every ten minutes. The readiness endpoint is `/api/health/ready`; it reports database/catalog status, price freshness, and the last result of each persistent sync job.
